...
def company_admin():
         with con:
            ...
            cur.execute(query1)
            rows= cur.fetchall()
         return render_template("com_admin.html",datas=rows, user=current_user)          

@auth.route('/job_admin')
def job_admin():
        with con:
            atitle = request.args.get('t')
            cur=con .cursor() 
            ...
            cur.execute(query1)
            rows= cur.fetchall()
        return render_template("job_admin.html",datas2=rows, user=current_user)

@auth.route("/add_com")
def add_com():
    return render_template("add_com.html", user=current_user)
